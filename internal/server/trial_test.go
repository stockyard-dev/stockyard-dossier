...
func TestTrialMiddleware_allowlistedWritesPass(t *testing.T) {
	...
	// of trial-required state. /api/import/preview is allowlisted because
	// it's read-only despite being a POST (it doesn't write to the DB).
	cases := []struct {
		method, path string
		body         []byte
	}{
		{"POST", "/api/license/activate", []byte(`{"license_key":"SY-bogus"}`)},
		{"POST", "/api/import/preview", []byte(`{"csv":"Name,Email\nA,a@x.com\n"}`)},
	}
	for _, c := range cases {
